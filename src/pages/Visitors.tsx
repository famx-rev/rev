    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-8 flex items-center justify-center">
        <div className="text-center">
          <Globe2 className="h-16 w-16 text-gray-300 dark:text-gray-600 mx-auto mb-4" />
          <h2 className="text-xl font-medium text-gray-900 dark:text-white mb-2">No Project Selected</h2>
          <p className="text-gray-500 dark:text-gray-400">Select a project from the sidebar to view visitors.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-3xl font-bold text-gray-800 dark:text-gray-100">Visitors</h1>
          <div className="flex items-center gap-3">
            <button
              onClick={() => setLiveOnly(v => !v)}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl font-medium text-sm transition-all shadow-sm border ${
                liveOnly
                  ? 'bg-emerald-500 dark:bg-emerald-600 border-emerald-600 dark:border-emerald-700 text-white shadow-emerald-200 dark:shadow-emerald-900/50'
                  : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:border-emerald-400 dark:hover:border-emerald-500 hover:text-emerald-600 dark:hover:text-emerald-400'
              }`}
            >
              ...
              <span className={`inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 rounded-full text-xs font-bold ${
                liveOnly ? 'bg-white/30 text-white' : 'bg-emerald-100 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400'
              }`}>
                {liveCount}
              </span>
            </button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 bg-white dark:bg-gray-900 rounded-2xl shadow-sm overflow-hidden">
                <div className="p-4 border-b border-gray-100 dark:border-gray-800 flex flex-wrap items-center gap-3">
                  <div className="relative flex-1 min-w-[200px]">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-3 py-2 text-sm border border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 dark:placeholder-gray-500 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400"
                    />
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      className="text-sm border border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 rounded-lg px-2 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400"
                    >
                ...
                <div className="divide-y divide-gray-100 dark:divide-gray-800 max-h-[560px] overflow-y-auto">
                  {filtered.length === 0 && (
                    <div className="p-12 text-center text-gray-400 dark:text-gray-500">No visitors match your filters.</div>
                  )}
                  {filtered.map(v => {
                    ...
                    return (
                      <div
                        ...
                        className="w-full text-left p-4 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors flex items-center gap-4 cursor-pointer group"
                      >
                        <div className="relative w-10 h-10 rounded-full bg-indigo-50 dark:bg-indigo-950/40 flex items-center justify-center flex-shrink-0 overflow-hidden">
                          {v.flag ? (
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="font-medium text-gray-900 dark:text-white truncate">{v.ip || 'Unknown IP'}</span>
                            {v.loading && <Loader2 className="h-3.5 w-3.5 animate-spin text-gray-400 dark:text-gray-500" />}
                            {isLive(v.lastSeen) && (
                              <span className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400 text-xs font-medium rounded-full">
                        ...
                        <div className="text-right">
                          <span className="inline-block px-2 py-0.5 text-xs font-medium bg-gray-100 dark:bg-gray-800 dark:text-gray-300 rounded-full">{v.eventCount} events</span>
                        </div>
                        <button
                          onClick={(e) => { e.stopPropagation(); setSelected(v); }}
                          className="p-2 rounded-lg text-gray-400 dark:text-gray-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/40 transition-colors flex-shrink-0"
...
function StatCard({ icon, label, value, color }: { icon: React.ReactNode; label: string; value: number; color: string }) {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 p-5 rounded-xl shadow-sm flex items-center gap-4">
...
function VisitorDrawer({ visitor, onClose }: { visitor: VisitorRow; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-gray-900 h-full overflow-y-auto shadow-2xl" style={{ animation: 'slideIn 0.2s ease-out' }}>
        <div className="sticky top-0 bg-gradient-to-r from-indigo-600 dark:from-indigo-700 to-indigo-500 dark:to-indigo-600 text-white p-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
        ...
        <div className="p-5">
          ...
          {info.error && (
            <div className="text-sm text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-lg p-3 mb-4">
              Could not resolve this IP's location (free API limit or private IP).
            </div>
          )}

          <h3 className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1">Location</h3>
          ...
          <h3 className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1 mt-4">Network</h3>
          ...
          <h3 className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1 mt-4">Device & Browser</h3>
          ...
          <Row icon={<Fingerprint className="h-4 w-4" />} label="Visitor ID" value={<code className="text-xs dark:text-gray-300">{info.visitorId}</code>} />

          <h3 className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-1 mt-4">Activity</h3>
          ...
          {info.latitude && (
            <a
              ...
              className="mt-4 flex items-center justify-center gap-2 w-full py-2.5 bg-indigo-600 dark:bg-indigo-500 text-white rounded-lg hover:bg-indigo-700 dark:hover:bg-indigo-600 transition-colors"
            >
